...
countries <- vroom::vroom("series/posts/cumulative_effects/eia_vs_ebm/cea_countries.csv")
...
x <- key |>
  group_by(uid) |>
  filter(!key %in% c("ebm", "eia", "multiple stressors"))


# Check that they all have eia or ebm tag (remove this once completed)
x <- group_by(key, by = uid) |>
  summarize(check = "eia" %in% key | "ebm" %in% key)
